/**
 * Retreives meta data (versions, author, repo, etc) from a composer repository for specific packages
 * @type {JavaScript}
 */
...
var composerPackages = {
   ...
   * Retreives meta data (versions, author, repo, etc) from a composer repository for specific packages
   * 
   * @param  {String} repository URL to a composer repository (identical to how it's defined in the composer file)
   * @param  {Array}  packages   An array of packages to retrieve data for
   * @return {Object}            A promise containing returned package data or an error
   */
...
/**
 * Exports composerPackages.init() with a promise in a function for external use 
 * 
 * @implements composerPackages.init()
 */
module.exports = function(repository, packages) {
  return new Promise((resolve, reject) => {
    composerPackages.init(repository, packages)
      .then(resolve)
      .catch(reject)
  })
}
